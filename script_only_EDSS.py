...
print('Only use EDSS in clustering')

patient_traj_list_pc = []
for i in range(len(patient_traj_list_)):
    patient_traj_pc = patient_traj_list_[i][:, 0]
...
print(np.shape(Z))
cluster_ind = fcluster(Z, t=10, criterion='distance')
print(cluster_ind)

color_list = "rbgcmy"
for i in range(len(patient_traj_list_pc)):
    x_axis_array = np.linspace(0, 1, np.size(patient_traj_list_pc[i]))
    plt.plot(x_axis_array, patient_traj_list_pc[i].reshape(-1), c=color_list[cluster_ind[i]-1], alpha=0.2)
plt.ylabel('EDSS')
plt.xticks([])
plt.xlabel('')
plt.show()
