...
df = pd.DataFrame(big_mat, columns=values_parsed)
print(df.head())

num_comp = 2

mca = prince.MCA(n_components=num_comp)
mca = mca.fit(df)
print(mca.eigenvalues_summary)
print(mca.column_contributions_)
# transform into patient_traj_list_mca
df_trans = mca.transform(df)
big_mat_trans = df_trans.to_numpy().reshape((-1, num_comp))
patient_list_mca = []
for traj_ind in range(int(np.max(traj_ind_in_bm))+1):
    patient_traj = big_mat_trans[traj_ind_in_bm == traj_ind, :]
    patient_list_mca.append(patient_traj)

print(patient_list_mca[-10])

dist_mat = spms_patient_class.construct_traj_dist_mat(patient_list_mca, method='frechet', order=2, oversampling=5)
...
print(np.shape(Z))
cluster_ind = fcluster(Z, t=4.6, criterion='distance')  # 4.6
print(cluster_ind)

spms_patient_class.plot_all_traj_2d(patient_list_mca)

spms_patient_class.plot_all_traj_2d(patient_list_mca, cluster_ind=cluster_ind)
